...
link = config.get("IndecisionRecords", "link")
...
def process_artist(link, href):
    ...
    try:
        if artist_response.status_code == 200:
            ...
            try:
                artist_name = artist_soup.find('h2')
...
def data_IndecisionRecords(link):
    ...
    if response.status_code == 200:
        ...
        artist_columns = soup.find_all("div", class_="column grid-item")
        for artist_column in artist_columns:
            href = artist_column.find("a").get("href")
            artists.append(process_artist(link, href))
    
        # Saving the data to a json file
        with open('../NEW_DATA/IndecisionRecords.json', 'w') as f:
